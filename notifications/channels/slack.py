# notifications/channels/slack.py
#
# Purpose:
# Slack webhook dispatch for governance notifications.
#
# Uses Slack Block Kit for structured message formatting.
# No Slack SDK dependency — plain urllib.request only.
# Zero external dependencies beyond Python stdlib.
#
# IMPORTANT:
# This module NEVER raises exceptions to the caller.
# All failures are caught internally and return False.
# The dispatcher handles logging.

from __future__ import annotations

import json
import urllib.error
import urllib.request
from typing import Any

from cli.display import decision_icon

_TIMEOUT_SECONDS: int = 5

# Priority-to-emoji mapping for Slack message headers.
# Five distinct icons — no two priorities share the same emoji.
# Decision icons are imported from cli.display (single source of truth).
_PRIORITY_EMOJI: dict[str, str] = {
    "urgent": "🚨",
    "high": "⚠️",
    "medium": "🔔",
    "low": "🔵",
    "informational": "ℹ️",
}


def send_slack(
    notification: dict[str, Any],
    webhook_url: str,
) -> bool:
    """
    Send a single governance notification to a Slack webhook.

    Uses Block Kit for structured formatting.

    Args:
        notification:  single notification dict from manifest
        webhook_url:   Slack incoming webhook URL

    Returns:
        True if webhook returned HTTP 200, False otherwise.
        Never raises.
    """
    try:
        if not webhook_url:
            return False

        role: str = notification.get("target_role", "unknown")
        body: str = notification.get("body", "")
        priority: str = notification.get("priority", "medium")
        decision: str = notification.get("decision", "")
        severity: str = notification.get("effective_severity", "")
        policy: str = notification.get("policy", "")
        requires: bool = notification.get("requires_action", False)

        header_emoji: str = _PRIORITY_EMOJI.get(priority, "ℹ️")
        decision_icon_str: str = decision_icon(decision)
        action_note: str = (
            "⚡ *Action required*" if requires else "Awareness notification"
        )

        payload: dict[str, Any] = {
            "blocks": [
                {
                    "type": "header",
                    "text": {
                        "type": "plain_text",
                        "text": f"{header_emoji} ObsidianWall Governance Alert",
                        "emoji": True,
                    },
                },
                {
                    "type": "section",
                    "fields": [
                        {
                            "type": "mrkdwn",
                            "text": f"*Policy:*\n`{policy}`",
                        },
                        {
                            "type": "mrkdwn",
                            "text": f"*Decision:*\n{decision_icon_str} `{decision}`",
                        },
                        {
                            "type": "mrkdwn",
                            "text": f"*Severity:*\n`{severity.upper()}`",
                        },
                        {
                            "type": "mrkdwn",
                            "text": f"*Intended for:*\n`{role}`",
                        },
                    ],
                },
                {
                    "type": "section",
                    "text": {
                        "type": "mrkdwn",
                        "text": body,
                    },
                },
                {
                    "type": "context",
                    "elements": [
                        {
                            "type": "mrkdwn",
                            "text": action_note,
                        }
                    ],
                },
                {
                    "type": "divider",
                },
            ]
        }

        data: bytes = json.dumps(payload).encode("utf-8")
        req = urllib.request.Request(
            webhook_url,
            data=data,
            headers={"Content-Type": "application/json"},
            method="POST",
        )

        with urllib.request.urlopen(req, timeout=_TIMEOUT_SECONDS) as response:
            return response.status == 200

    except Exception:
        # Never propagate — caller handles logging
        return False
