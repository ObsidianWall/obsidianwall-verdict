...
def _count_open_ingress_rules(resources: list[dict[str, Any]]) -> int:
    ...
    for resource in resources:
        ...
        if rtype == "azurerm_network_security_rule":
            if (values.get("direction") == "Inbound"
...
def _count_public_storage(resources: list[dict[str, Any]]) -> int:
    ...
    for resource in resources:
        ...
        if rtype == "aws_s3_bucket_public_access_block":
            if (values.get("block_public_acls") is True
...
def _count_compute_instances(resources: list[dict[str, Any]]) -> int:
    return sum(1 for r in resources
...
_AZURE_COMPUTE_TYPES = frozenset({"azurerm_virtual_machine","azurerm_linux_virtual_machine","azurerm_windows_virtual_machine"})
_AWS_COMPUTE_TYPES = frozenset({"aws_instance","aws_launch_template"})
_AZURE_GPU_VM_SIZES = frozenset({"Standard_NC6","Standard_NC12","Standard_NC6s_v3","Standard_ND6s","Standard_NV6"})
    ...
    for r in resources:
        ...
        if t == "azurerm_network_security_group":
            for rule in v.get("security_rule",[]):
                if rule.get("direction")=="Inbound" and rule.get("access")=="Allow" and rule.get("source_address_prefix") in _OPEN_INGRESS_SOURCES:
                    count += 1
        if t == "azurerm_network_security_rule":
            if v.get("direction")=="Inbound" and v.get("access")=="Allow" and v.get("source_address_prefix") in _OPEN_INGRESS_SOURCES:
                count += 1
        if t == "aws_security_group":
            for rule in v.get("ingress",[]):
                if "0.0.0.0/0" in rule.get("cidr_blocks",[]): count += 1
    return count
...
def _count_public_storage(resources):
    ...
    for r in resources:
        t, v = r.get("type",""), r.get("values",{})
        if t == "azurerm_storage_account" and v.get("allow_blob_public_access") is True: count += 1
        if t == "aws_s3_bucket": count += 1
        if t == "aws_s3_bucket_public_access_block" and v.get("block_public_acls") is True: count = max(0, count-1)
    return count
...
def _count_unencrypted_db(resources):
    ...
    for r in resources:
        t, v = r.get("type",""), r.get("values",{})
        if t in ("azurerm_sql_database","azurerm_mssql_database") and v.get("transparent_data_encryption_enabled") is not True: count += 1
        if t in ("aws_db_instance","aws_rds_cluster") and v.get("storage_encrypted") is not True: count += 1
    return count
...
def _count_compute(resources):
    return sum(1 for r in resources if r.get("type") in _AZURE_COMPUTE_TYPES | _AWS_COMPUTE_TYPES)

def _count_gpu(resources):
    count = 0
    for r in resources:
        t, v = r.get("type",""), r.get("values",{})
        if t in _AZURE_COMPUTE_TYPES and v.get("vm_size","") in _AZURE_GPU_VM_SIZES: count += 1
        if t == "aws_instance" and any(v.get("instance_type","").startswith(p) for p in _AWS_GPU_PREFIXES): count += 1
    return count
